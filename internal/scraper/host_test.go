...
func TestMadaraEndToEnd(t *testing.T) {
	eachRuntime(t, func(t *testing.T, lr luaRuntime) {
		...
		h := &Host{LuaDir: dir}
		r, err := lr.open(h, context.Background(), writeModule(t, srv.URL), "", "")
		if err != nil {
			t.Fatal(err)
		}
		defer r.Close()
...
func TestOneFileDeclaresSeveralSites(t *testing.T) {
	eachRuntime(t, func(t *testing.T, lr luaRuntime) {
		...
		r, err = lr.open(h, context.Background(), path, "Gated Site", "")
		if err != nil {
			t.Fatal(err)
		}
		defer r.Close()
		...
		if _, err := lr.open(h, context.Background(), path, "Nowhere", ""); err == nil {
...
`
		...
		r, err := lr.open(h, context.Background(), path, "Many Doors", "")
		if err != nil {
			t.Fatal(err)
		}
		...
		r, err = lr.open(h, context.Background(), path, "Many Doors", "https://two.example")
		if err != nil {
			t.Fatal(err)
		}
		...
		r, err = lr.open(h, context.Background(), path, "Many Doors", "https://gone.example")
		if err != nil {
			t.Fatal(err)
		}
		defer r.Close()
