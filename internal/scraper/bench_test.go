...
// golden fixtures of the two templates that back the most modules. Every
// scrape opens a fresh runner, as the worker pool does.
//
//	ATSUME_FMD2_DIR=... go test ./internal/scraper/ -run '^$' -bench Scrape -benchmem
func BenchmarkScrape(b *testing.B) {
	benchmarkGolden(b, func(b *testing.B, r *Runner, c goldenCase, base string) {
		if c.directory {
...
func BenchmarkOpen(b *testing.B) {
	benchmarkGolden(b, func(*testing.B, *Runner, goldenCase, string) {})
}

func benchmarkGolden(b *testing.B, scrape func(b *testing.B, r *Runner, c goldenCase, base string)) {
